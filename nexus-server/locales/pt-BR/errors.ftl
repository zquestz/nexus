...
err-avatar-undecodable = Não foi possível decodificar o avatar como uma imagem válida
err-authentication = Erro de autenticação
err-invalid-credentials = Nome de usuário ou senha inválidos
err-login-rate-limited = Muitas tentativas de login malsucedidas. Tente novamente mais tarde.
...
err-personal-file-area-exists = A área pessoal de arquivos de "{ $username }" já existe
err-personal-file-area-migration-failed = Falha ao migrar a área pessoal de arquivos
err-personal-file-area-busy = A área pessoal de arquivos está ocupada
err-personal-file-area-rollback-failed-warning = A reversão da área pessoal de arquivos falhou; a área pode permanecer em "{ $new_username }" em vez de "{ $old_username }". Verifique os logs do servidor antes de tentar novamente.
...
err-permission-grant-revoke-conflict = A permissão { $permission } não pode ser concedida e revogada ao mesmo tempo
...
err-server-image-undecodable = Não foi possível decodificar a imagem do servidor como uma imagem válida
...
err-news-image-undecodable = Não foi possível decodificar a imagem da notícia como uma imagem válida
...
err-upload-insufficient-space = Espaço livre insuficiente para o upload
...
err-slow-client-disconnect = Desconectado: seu cliente não conseguiu acompanhar as mensagens do servidor.
