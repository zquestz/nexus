...
err-login-rate-limited = Demasiadas tentativas de início de sessão falhadas. Tente novamente mais tarde.
...
err-cannot-reset-privileged-password = Não pode repor a palavra-passe de um utilizador que tem permissões que não possui
...
err-personal-file-area-exists = A área pessoal de ficheiros de "{ $username }" já existe
err-personal-file-area-migration-failed = Falha ao migrar a área pessoal de ficheiros
err-personal-file-area-busy = A área pessoal de ficheiros está ocupada
err-personal-file-area-rollback-failed-warning = A reversão da área pessoal de ficheiros falhou; a área pode permanecer em "{ $new_username }" em vez de "{ $old_username }". Verifique os registos do servidor antes de tentar novamente.
...
err-server-image-undecodable = Não foi possível descodificar a imagem do servidor como uma imagem válida
...
err-news-image-undecodable = Não foi possível descodificar a imagem da notícia como uma imagem válida
...
err-source-busy = O ficheiro está em uso no momento. Por favor, tente novamente.
...
err-slow-client-disconnect = Desconectado: o seu cliente não conseguiu acompanhar as mensagens do servidor.
