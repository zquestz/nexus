...
err-nickname-unavailable = ニックネームは使用できません
...
err-personal-file-area-exists = 「{ $username }」の個人ファイル領域は既に存在します
err-personal-file-area-migration-failed = 個人ファイル領域の移行に失敗しました
err-personal-file-area-busy = 個人ファイル領域は使用中です
err-personal-file-area-rollback-failed-warning = 個人ファイル領域のロールバックに失敗しました。ファイル領域が「{ $old_username }」ではなく「{ $new_username }」に残っている可能性があります。再試行する前にサーバーログを確認してください。
...
err-server-image-undecodable = サーバー画像を有効な画像としてデコードできませんでした
...
err-news-image-undecodable = ニュース画像を有効な画像としてデコードできませんでした
