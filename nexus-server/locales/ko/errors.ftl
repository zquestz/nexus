...
err-personal-file-area-exists = "{ $username }"의 개인 파일 영역이 이미 존재합니다
err-personal-file-area-migration-failed = 개인 파일 영역을 마이그레이션하지 못했습니다
err-personal-file-area-busy = 개인 파일 영역이 사용 중입니다
err-personal-file-area-rollback-failed-warning = 개인 파일 영역 롤백에 실패했습니다. 파일 영역이 "{ $old_username }" 대신 "{ $new_username }" 아래에 남아 있을 수 있습니다. 다시 시도하기 전에 서버 로그를 확인하세요.
...
err-server-image-undecodable = 서버 이미지를 유효한 이미지로 디코딩할 수 없습니다
...
err-news-image-undecodable = 뉴스 이미지를 유효한 이미지로 디코딩할 수 없습니다
...
err-move-failed = 파일 또는 폴더를 이동할 수 없습니다
err-copy-failed = 파일 또는 폴더를 복사할 수 없습니다
err-destination-exists = 해당 이름의 파일 또는 디렉토리가 대상 위치에 이미 존재합니다
...
err-upload-insufficient-space = 업로드를 위한 여유 공간이 부족합니다
...
err-ban-invalid-target = 잘못된 대상 (닉네임, IP 주소 또는 CIDR 범위 사용)
err-target-too-long = 대상이 너무 깁니다 (최대 { $max_length }자)
...
err-tracker-rate-limited = 트래커가 요청 속도를 제한했습니다
