...
err-address-too-long = Адрес слишком длинный (максимум { $max_length } байт)
err-address-contains-scheme = Адрес не должен содержать URL-схему
err-address-contains-brackets = Адрес не должен содержать скобки
err-address-contains-path = Адрес не должен содержать путь
err-address-contains-userinfo = Адрес не должен содержать имя пользователя
err-address-contains-whitespace = Адрес не должен содержать пробелы
err-address-contains-port = Адрес не должен содержать порт
err-address-contains-zone-id = Адрес не должен содержать идентификатор зоны IPv6
err-address-invalid-format = Адрес не является допустимым именем хоста или IP-адресом
err-address-empty = Адрес не может быть пустым
