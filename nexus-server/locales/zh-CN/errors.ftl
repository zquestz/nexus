...
err-nickname-unavailable = 昵称不可用
...
err-cannot-reset-privileged-password = 您无法重置拥有您不具备的权限的用户的密码
...
err-personal-file-area-exists = “{ $username }”的个人文件区已存在
err-personal-file-area-migration-failed = 个人文件区迁移失败
err-personal-file-area-busy = 个人文件区正忙
err-personal-file-area-rollback-failed-warning = 个人文件区回滚失败；文件区可能仍位于“{ $new_username }”而不是“{ $old_username }”。重试前请检查服务器日志。
...
err-server-image-undecodable = 无法将服务器图片解码为有效图像
...
err-news-image-undecodable = 无法将新闻图片解码为有效图像
...
err-tracker-rate-limited = 受到追踪器速率限制
