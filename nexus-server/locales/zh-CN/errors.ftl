...
err-address-too-long = 地址太长（最多{ $max_length }个字节）
err-address-contains-scheme = 地址不能包含URL协议
err-address-contains-brackets = 地址不能包含方括号
err-address-contains-path = 地址不能包含路径
err-address-contains-userinfo = 地址不能包含用户名
err-address-contains-whitespace = 地址不能包含空格
err-address-contains-port = 地址不能包含端口
err-address-contains-zone-id = 地址不能包含IPv6区域标识符
err-address-invalid-format = 地址不是有效的主机名或IP地址
err-address-empty = 地址不能为空
