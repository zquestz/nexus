...
err-status-too-long = El mensaje de ausencia es demasiado largo (máx. { $max_length } bytes)
...
err-broadcast-too-long = Mensaje demasiado largo (máx. { $max_length } bytes)
err-chat-too-long = Mensaje demasiado largo (máx. { $max_length } bytes)
err-topic-too-long = El tema no puede exceder { $max_length } bytes
...
err-server-name-too-long = El nombre del servidor es demasiado largo (máx. { $max_length } bytes)
...
err-server-description-too-long = La descripción del servidor es demasiado larga (máx. { $max_length } bytes)
...
err-address-too-long = La dirección es demasiado larga (máx. { $max_length } bytes)
err-address-contains-scheme = La dirección no debe incluir un esquema de URL
err-address-contains-brackets = La dirección no debe incluir corchetes
err-address-contains-path = La dirección no debe incluir una ruta
err-address-contains-userinfo = La dirección no debe incluir un nombre de usuario
err-address-contains-whitespace = La dirección no debe contener espacios en blanco
err-address-contains-port = La dirección no debe incluir un puerto
err-address-contains-zone-id = La dirección no debe incluir un identificador de zona IPv6
err-address-invalid-format = La dirección no es un nombre de host o dirección IP válida
err-address-empty = La dirección no puede estar vacía

# Errores de noticias
err-news-not-found = Noticia #{ $id } no encontrada
err-news-body-too-long = El contenido de la noticia es demasiado largo (máx. { $max_length } bytes)
