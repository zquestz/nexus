...
err-address-too-long = 位址太長（最多{ $max_length }個位元組）
err-address-contains-scheme = 位址不能包含URL協定
err-address-contains-brackets = 位址不能包含方括號
err-address-contains-path = 位址不能包含路徑
err-address-contains-userinfo = 位址不能包含使用者名稱
err-address-contains-whitespace = 位址不能包含空格
err-address-contains-port = 位址不能包含連接埠
err-address-contains-zone-id = 位址不能包含IPv6區域識別碼
err-address-invalid-format = 位址不是有效的主機名稱或IP位址
err-address-empty = 位址不能為空
