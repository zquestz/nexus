...
err-nickname-unavailable = 暱稱不可用
...
err-login-rate-limited = 登入失敗次數過多。請稍後再試。
...
err-admin-cannot-have-group = 無法將管理員用戶指派到群組
...
err-personal-file-area-exists = 「{ $username }」的個人檔案區已存在
err-personal-file-area-migration-failed = 個人檔案區遷移失敗
err-personal-file-area-busy = 個人檔案區忙碌中
err-personal-file-area-rollback-failed-warning = 個人檔案區回復失敗；檔案區可能仍位於「{ $new_username }」而不是「{ $old_username }」。重試前請檢查伺服器記錄。
...
err-invalid-password-strength = 無效的密碼強度值
...
err-server-image-undecodable = 無法將伺服器圖片解碼為有效圖像
...
err-news-image-undecodable = 無法將新聞圖片解碼為有效圖像
...
err-move-failed = 無法移動檔案或目錄
err-copy-failed = 無法複製檔案或目錄
err-destination-exists = 目的地已存在同名的檔案或目錄
...
err-transfer-path-invalid = 路徑包含無效字元
err-transfer-access-denied = 存取被拒絕
...
err-upload-insufficient-space = 上傳可用空間不足
...
err-upload-protocol-error = 上傳協議錯誤
err-upload-connection-lost = 上傳過程中連線中斷
...
err-group-name-too-long = 群組名稱太長（最多{ $max_length }個字元）
...
err-tracker-too-many = 已達追蹤器上限 (最多 { $max } 個)

# Tracker registration status messages
err-tracker-connection-failed = 無法連線到追蹤器
err-tracker-tls-failed = 與追蹤器的 TLS 交握失敗
err-tracker-handshake-failed = 追蹤器交握失敗
err-tracker-connection-lost = 與追蹤器的連線中斷
err-tracker-db-failed = 更新追蹤器狀態時發生資料庫錯誤
err-tracker-fingerprint-mismatch = 追蹤器憑證與已儲存的指紋不符
err-tracker-fingerprint-intercepted = 追蹤器自我回報的指紋與其 TLS 憑證不符
err-tracker-unauthorized = 追蹤器拒絕了註冊
err-tracker-rate-limited = 受到追蹤器速率限制
...
err-slow-client-disconnect = 已斷開連線：您的用戶端無法跟上伺服器訊息。

# Bandwidth Errors
err-bandwidth-weight-delegation = 無法授予高於自己的頻寬權重
err-bandwidth-weight-inherit-would-elevate = 無法繼承高於自己的頻寬權重
err-bandwidth-weight-zero = 頻寬權重必須至少為 { $min }
err-bandwidth-chunk-size-too-small = 排程器區塊大小必須至少為 { $min } 位元組
err-bandwidth-chunk-size-too-large = 排程器區塊大小最多為 { $max } 位元組
