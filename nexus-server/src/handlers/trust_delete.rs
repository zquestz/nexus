    ...
    W: AsyncWrite + Unpin,
{
    ...
    if !requesting_user.has_permission(Permission::TrustDelete) {
        ...
        return ctx
            .send_message(&reject_trust_delete(err_permission_denied(ctx.locale)))
            .await;
    }
    ...
    if let Err(e) = validators::validate_target(&target) {
        ...
        return ctx.send_message(&reject_trust_delete(error_msg)).await;
    }

    let response = delete_trust_rules(ctx, &target, &requesting_user.username).await;

    ctx.send_message(&response).await
}

async fn delete_trust_rules<W>(
    ...
    W: AsyncWrite + Unpin,
{
    ...
    match ctx.db.trusts.delete_trusts_by_nickname(target).await {
        Ok(deleted_ips) if !deleted_ips.is_empty() => {
            ...
            info!(user = %requested_by, ip = %ctx.peer_addr, target = %target, "{}", LOG_TRUST_DELETE_SUCCESS);
            ...
            trust_delete_success(deleted_ips, Some(target.to_string()))
        }
        Ok(_) => delete_trust_by_ip_or_range(ctx, target, requested_by).await,
        Err(e) => {
            error!(user = %requested_by, ip = %ctx.peer_addr, target = %target, err = %e, "{}", LOG_TRUST_DELETE_DB_ERROR_NICKNAME);
            reject_trust_delete(super::err_database(ctx.locale))
        }
    }
}

async fn delete_trust_by_ip_or_range<W>(
    ctx: &HandlerContext<'_, W>,
    target: &str,
    requested_by: &str,
) -> ServerMessage
    ...
    W: AsyncWrite + Unpin,
{
    let Some((canonical, net, is_range)) = canonicalize_target(target) else {
        return reject_trust_delete(err_trust_not_found(ctx.locale, target));
    };
    ...
    if is_range {
        match ctx.db.trusts.delete_trusts_in_range(&net).await {
            Ok(all_deleted) => {
                ...
                info!(user = %requested_by, ip = %ctx.peer_addr, target = %target, "{}", LOG_TRUST_DELETE_SUCCESS);
                trust_delete_success(all_deleted, None)
            }
            Err(e) => {
                error!(user = %requested_by, ip = %ctx.peer_addr, target = %target, err = %e, "{}", LOG_TRUST_DELETE_DB_ERROR_CIDR);
                reject_trust_delete(super::err_database(ctx.locale))
            }
        }
    } else {
        match ctx.db.trusts.delete_trust_by_ip(&canonical).await {
            Ok(true) => {
                ...
                info!(user = %requested_by, ip = %ctx.peer_addr, target = %target, "{}", LOG_TRUST_DELETE_SUCCESS);
                trust_delete_success(vec![canonical], None)
            }
            Ok(false) => reject_trust_delete(err_trust_not_found(ctx.locale, target)),
            Err(e) => {
                error!(user = %requested_by, ip = %ctx.peer_addr, target = %target, err = %e, "{}", LOG_TRUST_DELETE_DB_ERROR_IP);
                reject_trust_delete(super::err_database(ctx.locale))
            }
        }
    }
...
fn trust_delete_success(ips: Vec<String>, nickname: Option<String>) -> ServerMessage {
...
fn reject_trust_delete(error: String) -> ServerMessage {
    ServerMessage::TrustDeleteResponse {
        success: false,
        error: Some(error),
        ips: None,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_trustdelete_echoes_typed_nickname_not_fold() {
        let mut test_ctx = create_test_context().await;
        let session_id = login_user(&mut test_ctx, "admin", "password", &[], true).await;

        // Trust annotated with a mixed-case nickname (display in `nickname`,
        ...
        match read_server_message(&mut test_ctx).await {
            ...
            other => panic!("Expected TrustDeleteResponse, got: {other:?}"),
        }
    }
