...
use crate::constants::{
    LOG_TRACKER_EDIT_DB_ERROR, LOG_TRACKER_EDIT_NOT_LOGGED_IN, LOG_TRACKER_EDIT_PERMISSION_DENIED,
};
    ...
    W: AsyncWrite + Unpin,
{
    let Some(session_id) = session_id else {
        ...
        return ctx
            .send_error_and_disconnect(&err_not_logged_in(ctx.locale), Some("TrackerEdit"))
            .await;
    };
    ...
    let requesting_user = match ctx.user_manager.get_user_by_session_id(session_id).await {
        ...
        None => {
            return ctx
                .send_error_and_disconnect(&err_authentication(ctx.locale), Some("TrackerEdit"))
                .await;
        }
    };
    ...
    let row = match ctx.db.trackers.get_by_id(id).await {
        ...
        Err(e) => {
            ...
            return ctx
                .send_error_and_disconnect(&err_database(ctx.locale), Some("TrackerEdit"))
                .await;
        }
    };
