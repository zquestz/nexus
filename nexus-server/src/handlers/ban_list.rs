    ...
    W: AsyncWrite + Unpin,
{
    ...
    let Some(session_id) = session_id else {
        ...
        return ctx
            .send_error_and_disconnect(&err_not_logged_in(ctx.locale), Some(HANDLER_BAN_LIST))
            .await;
    };
    ...
    let requesting_user = match ctx.user_manager.get_user_by_session_id(session_id).await {
        ...
        None => {
            return ctx
                .send_error_and_disconnect(&err_authentication(ctx.locale), Some(HANDLER_BAN_LIST))
                .await;
        }
    };
