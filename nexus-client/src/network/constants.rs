...
/// Post-login BBS writer progress timeout (60 seconds).
///
/// Large client-originated frames, such as news or server image updates,
/// may take longer than 60 seconds on slow uplinks. The writer therefore
...
pub const BBS_WRITE_PROGRESS_TIMEOUT: Duration = Duration::from_secs(60);
...
/// Post-login BBS reader progress timeout (60 seconds).
///
...
pub const BBS_READ_PROGRESS_TIMEOUT: Duration = Duration::from_secs(60);

/// Whole-operation deadline for gracefully closing a BBS connection (30 seconds).
pub const BBS_SHUTDOWN_TIMEOUT: Duration = Duration::from_secs(30);
...
mod tests {
    ...
    #[test]
    fn bbs_progress_deadlines_are_independent_of_setup_and_shutdown() {
        assert_eq!(BBS_READ_PROGRESS_TIMEOUT, Duration::from_secs(60));
        assert_eq!(BBS_WRITE_PROGRESS_TIMEOUT, Duration::from_secs(60));
        assert_eq!(BBS_RESPONSE_TIMEOUT, Duration::from_secs(30));
        assert_eq!(BBS_SHUTDOWN_TIMEOUT, Duration::from_secs(30));
    }
}
