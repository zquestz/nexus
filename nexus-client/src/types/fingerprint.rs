...
/// content>)`. Apply at every assignment site for any field that holds a
/// fingerprint pin so the in-memory and on-disk shape stays consistent —
/// Stage 1 TOFU then never has to disambiguate "empty string" from "no pin".
