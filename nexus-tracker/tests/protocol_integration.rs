...
/// `make_register`'s `"0.8.3"` default). Used by the compat-filter tests
...
async fn test_list_filters_incompatible_versions() {
    ...
    let registers = [
        ...
        make_register_with_version("Match BBS", "0.8.3"),
    ...
    match read_tracker_server(&mut c_reader).await {
        TrackerServerMessage::TrackerServerListResponse {
            ...
            assert_eq!(servers[0].version, "0.8.3");
        }
...
async fn test_list_rejects_over_cap_client_version() {
    ...
    let over_cap_version = "0.8.3".to_string() + &"a".repeat(64);
    send_tracker_client(
        &mut c_writer,
        &TrackerClientMessage::TrackerServerList {
            password: None,
