...
/// Maximum length for a tracker name in bytes.
///
/// Wider than `MAX_SERVER_NAME_LENGTH` (64) because tracker names are
/// admin-only metadata for distinguishing trackers in the management
/// UI — they're not user-facing chat decoration, so the tighter cap
/// doesn't apply. Aligned with `MAX_PASSWORD_LENGTH` (256) for a
/// generous practical cap.
pub const MAX_TRACKER_NAME_LENGTH: usize = 256;
...
/// - Does not exceed `MAX_TRACKER_NAME_LENGTH` bytes
...
pub fn validate_tracker_name(name: &str) -> Result<(), TrackerNameError> {
    ...
    if name.len() > MAX_TRACKER_NAME_LENGTH {
