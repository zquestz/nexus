...
| --------- | ------ | -------- | -------------------------------------------- |
| `version` | string | Yes      | Client's protocol version (e.g., `"0.9.10"`) |
...
```
NX|9|Handshake|a1b2c3d4e5f6|19|{"version":"0.9.10"}
```
...
**Fingerprint format.** The `fingerprint` field is the SHA-256 digest of the server's TLS certificate, encoded as 32 uppercase hex bytes separated by colons (95 bytes total — `AA:BB:CC:...`). Implementations MUST emit fingerprints in this canonical form so that comparisons can use byte equality without normalization.
...
```json
{
  ...
  "version": "0.9.10",
  "fingerprint": "AA:BB:CC:DD:EE:FF:00:11:22:33:44:55:66:77:88:99:AA:BB:CC:DD:EE:FF:00:11:22:33:44:55:66:77:88:99",
  "error": "Unsupported protocol version. Server: 0.9.10, Client: 0.3.0"
}
```
...
| ------ | ------ | ---------- | ------------------------ |
| 0.9.10 | 0.9.10 | ✅ Yes     | Exact match              |
| 0.9.3  | 0.9.10 | ✅ Yes     | Patch difference ignored |
| 0.8.0  | 0.9.10 | ❌ No      | Minor mismatch (pre-1.0) |
| 0.9.10 | 0.8.0  | ❌ No      | Minor mismatch (pre-1.0) |
| 1.0.0  | 0.9.10 | ❌ No      | Major version mismatch   |
| 0.9.10 | 1.0.0  | ❌ No      | Major version mismatch   |
